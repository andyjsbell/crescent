...
data class ProvisionSettings(   val clientId: String,
                                val claimCert: String,
                                val claimKey: String,
                                val rootCert: String,
                                val endpoint: String,
                                val template: String,
...
open class ProvisioningClient(private val settings: ProvisionSettings) {
                    ...
                    ClientBootstrap(eventLoopGroup, resolver).use { clientBootstrap ->
                        AwsIotMqttConnectionBuilder.newMtlsBuilder(settings.claimCert, settings.claimKey).use { builder ->
                            ...
                            builder.build().use { connection ->
                                ...
                                for (topic in topics) {
                                    val sub: CompletableFuture<Int> = connection.subscribe(
                                            topic,
                                        ...
                                        try {
                                            ...
                                            when {
                                                ...
                                                payload.contains("deviceConfiguration") -> {
                                                    Log.d(TAG, "deviceConfiguration: $payload")
                                                    var map: Map<String, Any> = HashMap()
                                                    map = Gson().fromJson(payload, map.javaClass)
                                                    val thingName = map["thingName"].toString()
                                                    Log.d(TAG, "Created thing with name: $thingName")
                                                    completed = true
                                                }
                                            }
                                ...
                                val published: CompletableFuture<Int> = connection.publish(
                                        MqttMessage(topic, "{}".toByteArray()),
                                        QualityOfService.AT_LEAST_ONCE,
                                        false
                                )
                                published.get()
                                ...
                                while (!completed) {
                                    ...
                                    if (ownershipToken != null) {
                                        // register the device
                                        val registerTemplate = "{\"certificateOwnershipToken\":\"${ownershipToken}\", \"parameters\":{\"SerialNumber\":\"${settings.clientId}\"}}"
                                        ownershipToken = null
